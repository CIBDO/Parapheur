...
const items = ref<MeetingTypeItem[]>([])
...
const dialogTitle = computed(() =>
  editingId.value ? 'Modifier le type' : 'Nouveau type de réunion',
)
...
const load = async () => {
  loading.value = true
  try {
    items.value = await $api('/meeting-types')
  }
  catch (e: any) {
...
const save = async () => {
  ...
  try {
    if (editingId.value) {
      await $api(`/meeting-types/${editingId.value}`, { method: 'PUT', body: form.value })
      successMessage.value = 'Type mis à jour'
    }
    else {
      await $api('/meeting-types', { method: 'POST', body: form.value })
...
const remove = async (item: MeetingTypeItem) => {
  if (!confirm(`Supprimer le type « ${item.code} » ?`))
    return
  try {
    await $api(`/meeting-types/${item.id}`, { method: 'DELETE' })
...
<template>
  <div>
    <ParapheurPageHeader
      ...
      subtitle="Référentiel des natures de réunions (comité, commission, atelier…)"
