...
const resolvePaymentStatus = (payment: number) => {
  if (payment === 1) return { text: 'Paid', color: 'success' };
  if (payment === 2) return { text: 'Pending', color: 'warning' };
  if (payment === 3) return { text: 'Cancelled', color: 'secondary' };
  if (payment === 4) return { text: 'Failed', color: 'error' };
};

const resolveStatus = (status: string) => {
  if (status === 'Delivered') return { text: 'Delivered', color: 'success' };
  if (status === 'Out for Delivery') return { text: 'Out for Delivery', color: 'primary' };
  if (status === 'Ready to Pickup') return { text: 'Ready to Pickup', color: 'info' };
...
<template>
  <div v-if="orderData">
      ...
      <div>
        <div class="d-flex gap-2 align-center mb-2 flex-wrap">
          ...
          <div class="d-flex gap-x-2">
            <VChip v-if="orderData?.payment" variant="tonal" :color="resolvePaymentStatus(orderData.payment)?.color" label size="small">
              {{ resolvePaymentStatus(orderData.payment)?.text }}
            </VChip>
            <VChip v-if="orderData?.status" v-bind="resolveStatus(orderData?.status)" label size="small" />
          </div>
        </div>
        <div class="text-body-1">Aug 17, 2020, 5:48 (ET)</div>
      </div>

      <VBtn variant="tonal" color="error" @click="isConfirmDialogVisible = !isConfirmDialogVisible"> Delete Order </VBtn>
    </div>
      ...
      <VCol cols="12" md="8">
        ...
        <VCard class="mb-6">
          <VCardItem>
            ...
            <template #append>
              <div class="text-base font-weight-medium text-primary cursor-pointer">Edit</div>
            </template>
          </VCardItem>

          <VDivider />
          <VDataTable :headers="headers" :items="orderDetail" item-value="productName" show-select class="text-no-wrap">
            ...
            <div class="d-flex align-end flex-column">
              <table class="text-high-emphasis">
                <tbody>
                  ...
                  <tr>
                    <td>Tax:</td>
                    <td class="font-weight-medium">$28</td>
                  </tr>
                  <tr>
            ...
            <VTimeline truncate-line="both" line-inset="9" align="start" side="end" line-color="primary" density="compact">
              <VTimelineItem dot-color="primary" size="x-small">
                <div class="d-flex justify-space-between align-center">
                  <div class="app-timeline-title">Order was placed (Order ID: #32543)</div>
                  <div class="app-timeline-meta">Tuesday 10:20 AM</div>
                </div>
                <p class="app-timeline-text mb-0 mt-3">Your order has been placed successfully</p>
              </VTimelineItem>
              ...
              <VTimelineItem dot-color="primary" size="x-small">
                ...
                <p class="app-timeline-text mb-0 mt-3">Pick-up scheduled with courier</p>
              </VTimelineItem>
              ...
              <VTimelineItem dot-color="primary" size="x-small">
                ...
                <p class="app-timeline-text mb-0 mt-3">Item has been picked up by courier.</p>
              </VTimelineItem>
              ...
              <VTimelineItem dot-color="primary" size="x-small">
                ...
                <p class="app-timeline-text mb-0 mt-3">Package arrived at an Amazon facility, NY</p>
              </VTimelineItem>
              ...
              <VTimelineItem dot-color="primary" size="x-small">
                ...
                <p class="app-timeline-text mb-0 mt-3">Package has left an Amazon facility , NY</p>
              </VTimelineItem>
              ...
              <VTimelineItem dot-color="secondary" size="x-small">
                ...
                <p class="app-timeline-text mb-4 mt-3">Package will be delivered by tomorrow</p>
              </VTimelineItem>
            </VTimeline>
          </VCardText>
        </VCard>
        ...
        <VCard class="mb-6">
          <VCardText class="d-flex flex-column gap-y-6">
            ...
            <div class="d-flex align-center">
              <VAvatar v-if="orderData" :variant="!orderData?.avatar.length ? 'tonal' : undefined" :rounded="1" class="me-3">
              ...
              <div>
                ...
                <div class="text-body-1">Customer ID: #{{ orderData?.order }}</div>
              </div>
            </div>
        ...
        <VCard>
          <VCardText>
            ...
            <div class="mt-6">
              ...
              <div class="text-body-1">Card Number: ******{{ orderData?.methodNumber }}</div>
            </div>
          </VCardText>
        </VCard>
      </VCol>
    ...
    <UserInfoEditDialog v-model:is-dialog-visible="isUserInfoEditDialogVisible" :user-data="userData" />

    <AddEditAddressDialog v-model:is-dialog-visible="isEditAddressDialogVisible" :billing-address="currentBillingAddress" />
  </div>
  <section v-else>
    <VAlert type="error" variant="tonal"> Order with ID #{{ route.params.id }} is not available or could not be found! </VAlert>
  </section>
</template>
