            ...
            <div class="flex-grow-1">
              ...
              >
                ...
                <div class="flex-grow-1" style="min-inline-size: 150px">
                  <VProgressLinear color="primary" height="8" :model-value="(review.value / 185) * 100" rounded />
                </div>
        ...
        <VCardText>
          <div class="d-flex justify-space-between flex-sm-row flex-column">
            <div>
              ...
              <div>
                <div class="text-high-emphasis text-body-1 mb-2"><span class="text-success">87%</span> Positive Reviews</div>
      ...
      <VCard>
        <VCardText>
          <div class="d-flex justify-space-between flex-wrap gap-6">
            <div>
              <AppTextField v-model="searchQuery" style="max-inline-size: 200px; min-inline-size: 200px" placeholder="Search Review" />
            </div>
            <div class="d-flex flex-row gap-4 align-center flex-wrap">
              ...
              />
              <VBtn prepend-icon="tabler-upload" variant="tonal" color="default"> Export </VBtn>
            </div>
          </div>
        </VCardText>
        ...
        >
          <template #item.product="{ item }">
            <div class="d-flex gap-x-4 align-center">
              <VAvatar :image="item.productImage" :size="38" variant="tonal" rounded />
          ...
          <template #item.review="{ item }">
            <div class="my-4">
              <VRating :id="item.id" :name="`${item.id}`" readonly :model-value="item.review" size="24" class="mb-1" />
          ...
          <template #item.status="{ item }">
            <VChip :color="item.status === 'Published' ? 'success' : 'warning'" label size="small">
              {{ item.status }}
            </VChip>
          </template>
          ...
          <template #item.actions="{ item }">
            <IconBtn>
              ...
              <VMenu activator="parent">
                <VList>
                  <VListItem value="view" :to="{ name: 'apps-ecommerce-order-details-id', params: { id: item.id } }"> View </VListItem>
                  <VListItem value="delete" @click="deleteReview(item.id)"> Delete </VListItem>
                </VList>
              </VMenu>
            </IconBtn>
          </template>
          ...
          <!-- pagination -->
          <template #bottom>
            <TablePagination v-model:page="page" :items-per-page="itemsPerPage" :total-items="totalReviews" />
          </template>
        </VDataTableServer>
      </VCard>
    </VCol>
