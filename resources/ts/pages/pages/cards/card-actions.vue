...
<template>
  <VRow>
    <VCol cols="12">
      <!-- 👉 Main Card  -->
      <AppCardActions title="Cards Actions" @refresh="refetchData">
        <VCardText>
          <VTable>
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="data in tableData" :key="data.icon">
                <td>
                  {{ data.action }}
                </td>
                <td>
                  <VIcon size="20" :icon="data.icon" />
                </td>
                <td>
                  {{ data.details }}
                </td>
    ...
    <VCol cols="12" md="6">
      <!-- 👉 Initial Load -->
      <AppCardActions v-model:loading="isLoading" title="Initial Load" no-actions>
        <VCardText>
    ...
    <VCol cols="12" md="6">
      <!-- 👉 Collapsible -->
      <AppCardActions action-collapsed title="Collapsible">
        <VCardText>
          <p>You can specifically add collapsible action using <code>actionCollapse</code> prop</p>
          <span>Click on <VIcon size="20" icon="tabler-chevron-up" /> icon to see it in action</span>
        </VCardText>
      </AppCardActions>
    </VCol>
    ...
    <VCol cols="12" md="6">
      <!-- 👉 Refresh Content -->
      <AppCardActions title="Refresh Content" action-refresh @refresh="refetchData">
        <VCardText>
          <p>You can specifically add refresh action using <code>actionRefresh</code> prop</p>
          <span>Click on <VIcon size="20" icon="tabler-refresh" /> icon to see it in action</span>
        </VCardText>
      </AppCardActions>
    </VCol>
    ...
    <VCol cols="12" md="6">
      <!-- 👉 Remove Card -->
      <AppCardActions title="Remove Card" action-remove>
        <VCardText>
          <p>You can specifically add remove action using <code>actionRemove</code> prop</p>
          <span>Click on <VIcon size="20" icon="tabler-x" /> icon to see it in action</span>
        </VCardText>
      </AppCardActions>
    </VCol>
    ...
    <VCol cols="12" md="6">
      <!-- 👉 Slots -->
      <AppCardActions title="Slots" @refresh="refetchData">
        <template #before-actions>
          <VChip class="me-3" color="primary" size="small"> 3 Updates </VChip>
        </template>
