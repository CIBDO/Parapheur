...
export const initConfigStore = () => {
  ...
  watch([() => configStore.theme, userPreferredColorScheme], () => {
    const themetoUpdate =
      configStore.theme === 'system' ? (userPreferredColorScheme.value === 'dark' ? 'dark' : 'light') : configStore.theme;

    vuetifyTheme.change(themetoUpdate);
  });

  onMounted(() => {
    if (configStore.theme === 'system') vuetifyTheme.change(userPreferredColorScheme.value);
  });
};
// !SECTION
