...
watch(
  ...
  () => {
    ...
    if (cookiePrimaryColor && !colors.some(color => color.main === cookiePrimaryColor)) customPrimaryColor.value = cookiePrimaryColor;
  },
  { immediate: true },
);
      ...
      <PerfectScrollbar tag="ul" :options="{ wheelPropagation: false }">
        ...
        <CustomizerSection title="Theming" :divider="false">
          ...
          <div
            ...
            :class="
              vuetifyTheme.global.name.value === 'light' && configStore.appContentLayoutNav === AppContentLayoutNav.Vertical
                ? 'd-flex'
                : 'd-none'
            "
          >
            <VLabel for="customizer-semi-dark" class="text-h6 text-high-emphasis"> Semi Dark Menu </VLabel>
