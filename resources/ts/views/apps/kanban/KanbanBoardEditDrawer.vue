...
<template>
  ...
  >
    <!-- 👉 Header -->
    <AppDrawerHeaderSection title="Edit Task" @cancel="$emit('update:isDrawerOpen', false)" />

    <VDivider />

    <PerfectScrollbar :options="{ wheelPropagation: false }" style="block-size: calc(100vh - 4rem)">
      <VForm v-if="localKanbanItem" ref="refEditTaskForm" @submit.prevent="updateKanbanItem">
        <VCardText class="kanban-editor-drawer">
          <VRow>
            <VCol cols="12">
              <AppTextField v-model="localKanbanItem.title" label="Title" :rules="[requiredValidator]" />
            </VCol>

            <VCol cols="12">
              <AppDateTimePicker v-model="localKanbanItem.dueDate" label="Due date" :config="config" />
            </VCol>

            <VCol cols="12">
              <AppSelect v-model="localKanbanItem.labels" :items="labelOptions" label="Label" multiple eager>
                <template #chip="{ item }">
            ...
            <VCol cols="12">
              <p class="mb-1 text-body-2 text-high-emphasis" style="line-height: 15px">Assigned</p>
            ...
            <VCol cols="12">
              <VFileInput v-model="fileAttached" prepend-icon="" multiple variant="outlined" label="No file chosen" clearable>
                <template #append>
            ...
            <VCol cols="12">
              <p class="text-body-2 text-high-emphasis mb-1" style="line-height: 15px">COMMENT</p>
            ...
            <VCol cols="12">
              ...
              <VBtn color="error" variant="tonal" @click="deleteKanbanItem"> Delete </VBtn>
            </VCol>
          </VRow>
        </VCardText>
      </VForm>
