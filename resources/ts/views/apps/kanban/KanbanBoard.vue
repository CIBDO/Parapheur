...
const validateBoardTitle = () => {
  return props.kanbanData.boards.some(board => boardTitle.value && board.title.toLowerCase() === boardTitle.value.toLowerCase())
...
<template>
  <div class="kanban-main-wrapper d-flex gap-4 h-100">
    <!-- 👉 kanban render  -->
    <div ref="kanbanWrapper" class="d-flex ga-6">
      <template v-for="kb in localKanbanData" :key="kb.id">
        <!-- 👉 kanban task render -->
        <KanbanItems
    ...
    <div class="add-new-form text-no-wrap" style="inline-size: 10rem">
      <h6 class="text-lg font-weight-medium cursor-pointer" @click="isAddNewFormVisible = !isAddNewFormVisible">
        <VIcon size="18" icon="tabler-plus" /> Add New
      </h6>

      <!-- 👉 Form -->
      <VForm v-if="isAddNewFormVisible" ref="refAddNewBoard" class="mt-4" validate-on="submit" @submit.prevent="addNewBoard">
        <div class="mb-4">
          <VTextField
            v-model="boardTitle"
        ...
        <div class="d-flex gap-3">
          ...
          <VBtn size="small" variant="tonal" color="secondary" type="reset" @click="hideAddNewForm"> Cancel </VBtn>
        </div>
      </VForm>
    </div>
  </div>
