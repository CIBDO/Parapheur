...
<template>
  <div class="kanban-board">
    ...
    <div class="kanban-board-header pb-4">
      <VForm v-if="isBoardNameEditing" ref="refKanbanBoardTitle" @submit.prevent="renameBoard">
        ...
        >
          <template #append-inner>
            <VIcon size="20" color="success" icon="tabler-check" class="me-1" @click="renameBoard" />

            <VIcon size="20" color="error" icon="tabler-x" @click="hideResetBoardNameForm" />
          </template>
        </VTextField>
      </VForm>
      ...
      <div v-else class="d-flex align-center justify-space-between">
        ...
        <div class="d-flex align-center">
          <VIcon class="drag-handler" size="20" icon="tabler-arrows-move" />

          <MoreBtn size="28" icon-size="20" class="text-high-emphasis" :menu-list="boardActions" item-props />
        </div>
      </div>
    </div>
    ...
    >
      <template v-for="id in localIds" :key="id">
        <KanbanCard
      ...
      <div class="add-new-form">
        <h6 class="text-base font-weight-regular cursor-pointer ms-4" @click="isAddNewFormVisible = !isAddNewFormVisible">
          <VIcon size="15" icon="tabler-plus" /> Add New Item
        </h6>

        <VForm v-if="isAddNewFormVisible" ref="refForm" class="mt-4" validate-on="submit" @submit.prevent="addNewItem">
          <div class="mb-4">
            <VTextarea
          ...
          <div class="d-flex gap-4 flex-wrap">
            <VBtn size="small" type="submit"> Add </VBtn>
            <VBtn size="small" variant="tonal" color="secondary" @click="hideAddNewForm"> Cancel </VBtn>
          </div>
        </VForm>
      </div>
    </div>
