...
<template>
  ...
  <VNavigationDrawer data-allow-mismatch temporary :model-value="!!props.email" location="right" :scrim="false" floating class="email-view">
    <template v-if="props.email">
      ...
      <div class="email-view-action-bar d-flex align-center text-medium-emphasis px-6 gap-x-1">
        ...
        <IconBtn>
          ...
          <VTooltip activator="parent" location="top"> Move to </VTooltip>
        ...
        <IconBtn>
          ...
          <VMenu activator="parent">
            <VList density="compact">
              <VListItem v-for="label in labels" :key="label.title" href="#" @click.stop="updateMailLabel(label.title)">
                <template #prepend>
      ...
      <PerfectScrollbar tag="div" class="mail-content-container flex-grow-1 pa-sm-12 pa-6" :options="{ wheelPropagation: false }">
        <VCard class="mb-4">
          <div class="d-flex align-start align-sm-center pa-6 gap-x-4">
            <VAvatar size="38">
              <VImg :src="props.email.from.avatar" :alt="props.email.from.name" />
            </VAvatar>
          ...
          <VCardText>
            ...
            <div class="text-body-1 font-weight-medium text-truncate mb-4">{{ props.email.from.name }},</div>
          ...
          <template v-if="props.email.attachments.length">
            ...
            <VCardText class="d-flex flex-column gap-y-4 pt-4">
              ...
              <div v-for="attachment in props.email.attachments" :key="attachment.fileName" class="d-flex align-center">
                <VImg :src="attachment.thumbnail" :alt="attachment.fileName" aspect-ratio="1" max-height="24" max-width="24" class="me-2" />
        ...
        <VCard v-if="showReplyBox">
          <VCardText>
            <h6 class="text-h6 mb-6">Reply to {{ email?.from.name }}</h6>
            <TiptapEditor v-model="emailReply" placeholder="Write your message..." />
            <div class="d-flex justify-end gap-4 pt-2 flex-wrap">
              ...
              <VBtn variant="text" color="secondary">
                <template #prepend>
                  <VIcon icon="tabler-paperclip" class="text-high-emphasis" size="16" />
                </template>
                Attachments
              </VBtn>
              <VBtn append-icon="tabler-send"> Send </VBtn>
            </div>
          </VCardText>
        </VCard>
      </PerfectScrollbar>
