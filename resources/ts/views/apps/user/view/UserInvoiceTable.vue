...
const resolveInvoiceStatusVariantAndIcon = (status: string) => {
  if (status === 'Partial Payment') return { variant: 'success', icon: 'tabler-check' };
  if (status === 'Paid') return { variant: 'warning', icon: 'tabler-chart-pie' };
  if (status === 'Downloaded') return { variant: 'info', icon: 'tabler-arrow-down' };
  if (status === 'Draft') return { variant: 'primary', icon: 'tabler-folder' };
  if (status === 'Sent') return { variant: 'secondary', icon: 'tabler-mail' };
  if (status === 'Past Due') return { variant: 'error', icon: 'tabler-info-circle' };
    ...
    <VCard id="invoice-list">
      ...
      >
        ...
        <template #item.id="{ item }">
          <RouterLink :to="{ name: 'apps-invoice-preview-id', params: { id: item.id } }"> #{{ item.id }} </RouterLink>
        </template>
        ...
        <template #item.trending="{ item }">
          <VTooltip>
            <template #activator="{ props }">
              <VAvatar :size="28" v-bind="props" :color="resolveInvoiceStatusVariantAndIcon(item.invoiceStatus).variant" variant="tonal">
