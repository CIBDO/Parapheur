  ...
  <VCard>
    <VCardText>
      <div class="d-flex flex-wrap justify-space-between align-center gap-4">
        <h5 class="text-h5 font-weight-medium">Courses you are taking</h5>
        <div>
          <AppTextField v-model="searchQuery" placeholder="Search Course" style="max-inline-size: 300px; min-inline-size: 300px" />
        </div>
      </div>
    </VCardText>
    ...
    >
      ...
      <template #item.progress="{ item }">
        <div class="d-flex align-center gap-x-4" style="inline-size: 15.625rem">
          ...
          <div class="w-100">
            <VProgressLinear color="primary" height="8" :model-value="Math.floor((item.completedTasks / item.totalTasks) * 100)" rounded />
          </div>
          <div class="text-body-2">{{ item.completedTasks }}/{{ item.totalTasks }}</div>
        </div>
      </template>
