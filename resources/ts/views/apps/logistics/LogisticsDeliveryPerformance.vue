    ...
    <VCardText>
      <VList class="card-list">
        <VListItem v-for="(data, index) in deliveryData" :key="index">
          ...
          <VListItemSubtitle>
            <div :class="data.change > 0 ? 'text-success' : 'text-error'" class="d-flex align-center gap-x-1">
              <VIcon :icon="data.change > 0 ? 'tabler-chevron-up' : 'tabler-chevron-down'" size="20" />
              <div>{{ data.change }}%</div>
            </div>
          </VListItemSubtitle>
