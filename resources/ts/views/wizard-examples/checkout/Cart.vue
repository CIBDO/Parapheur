...
<template>
  <VRow v-if="checkoutCartDataLocal">
    <VCol cols="12" lg="8">
      ...
      <h5 class="text-h5 my-4">My Shopping Bag ({{ checkoutCartDataLocal.cartItems.length }} Items)</h5>
      ...
      <div v-if="checkoutCartDataLocal.cartItems.length" class="border rounded">
        <template v-for="(item, index) in checkoutCartDataLocal.cartItems" :key="item.name">
          <div class="d-flex align-center gap-4 pa-6 position-relative flex-column flex-sm-row" :class="index ? 'border-t' : ''">
            ...
            <div class="d-flex w-100 flex-column flex-md-row">
              <div class="d-flex flex-column gap-y-2">
                ...
                <VRating density="compact" :model-value="item.rating" size="24" readonly />

                <AppTextField v-model="item.quantity" type="number" style="inline-size: 9.375rem" density="compact" />
              </div>

              <VSpacer />

              <div class="d-flex flex-column mt-5 text-start text-md-end" :class="$vuetify.display.mdAndDown ? 'gap-2' : 'gap-4'">
                <div class="d-flex text-base align-self-md-end">
                  <div class="text-primary">${{ item.price }}</div>
                  <div>/</div>
                  <div class="text-decoration-line-through">${{ item.discountPrice }}</div>
                </div>

                <div>
                  <VBtn variant="tonal" size="small"> move to wishlist </VBtn>
                </div>
              </div>
            </div>
          </div>
      ...
      >
        <a href="#" class="font-weight-medium">Add more products from wishlist</a>
        <VIcon icon="tabler-arrow-right" size="16" class="flip-in-rtl text-primary" />
      </div>
    </VCol>
    ...
    <VCol cols="12" lg="4">
      <VCard flat variant="outlined">
        <!-- 👉 payment offer -->
        <VCardText>
          <h6 class="text-h6 mb-4">Offer</h6>

          <div class="d-flex align-center gap-4 flex-wrap">
            <AppTextField v-model="checkoutCartDataLocal.promoCode" placeholder="Enter Promo Code" style="min-inline-size: 200px" />
          ...
          <div class="bg-var-theme-background rounded pa-6 mt-4">
            <h6 class="text-h6 mb-2">Buying gift for a loved one?</h6>
