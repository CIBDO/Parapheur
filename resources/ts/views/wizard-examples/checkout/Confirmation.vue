...
const resolveDeliveryMethod = computed(() => {
  if (props.checkoutData.deliverySpeed === 'overnight') return { method: 'Overnight Delivery', desc: 'In 1 business day.' };
  else if (props.checkoutData.deliverySpeed === 'express') return { method: 'Express Delivery', desc: 'Normally in 3-4 business days' };
...
<template>
  <section class="text-base">
    <div class="text-center">
      ...
      <p>Your order <span class="text-body-1 font-weight-medium text-high-emphasis">#1536548131</span> has been placed!</p>
      <p class="mb-0">
        We sent an email to <span class="text-body-1 font-weight-medium text-high-emphasis">john.doe@example.com</span> with your order
    ...
    <VRow>
      <VCol cols="12" md="9">
        ...
        <div class="border rounded">
          <template v-for="(item, index) in props.checkoutData.cartItems" :key="item.name">
            <div class="d-flex align-start gap-4 pa-6 position-relative flex-column flex-sm-row" :class="index ? 'border-t' : ''">
              <div>
                <VImg width="80" :src="item.image" />
              </div>

              <div class="d-flex w-100 justify-space-between gap-4" :class="$vuetify.display.width <= 700 ? 'flex-column' : 'flex-row'">
                <div>
                  ...
                  <VChip :color="item.inStock ? 'success' : 'error'" label size="small">
                ...
                <div class="d-flex text-base" :class="$vuetify.display.width <= 700 ? 'align-self-start' : 'align-self-center'">
                  <div class="text-primary">${{ item.price }}</div>
                  <div>/</div>
                  <div class="text-decoration-line-through text-disabled">${{ item.discountPrice }}</div>
                </div>
              </div>
            </div>
          </template>
        ...
        <div class="border rounded">
          <div class="border-b pa-6">
            ...
            <div class="d-flex justify-space-between text-base">
              <div class="text-high-emphasis">Charges</div>
              <div v-if="props.checkoutData.deliverySpeed === 'free'" class="d-flex align-center">
