    ...
    <VCardText>
      <VList class="card-list">
        <VListItem v-for="state in browserStates" :key="state.title">
          <template #prepend>
            <img :src="state.avatarImg" height="28" width="28" class="me-4" />
          </template>
          ...
          <template #append>
            <div class="text-body-1 me-4">{{ state.stats }}%</div>
            <VProgressCircular :model-value="state.stats" :color="state.progress" width="3" size="28" />
          </template>
        </VListItem>
      </VList>
    </VCardText>
