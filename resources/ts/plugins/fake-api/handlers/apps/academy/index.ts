...
export const handlerAppsAcademy = [
  ...
  http.get('/api/apps/academy/courses', ({ request }) => {
    ...
    const filteredCourses = db.courses.filter(course => {
      return (
        (course.courseTitle.toLowerCase().includes(queryLowered) || course.user.toLowerCase().includes(queryLowered)) &&
    ...
    if (sortByLocal) {
      ...
      if (sortByLocal === 'progress') {
        filteredCourses.sort((a, b) => {
          if (orderByLocal === 'asc') return a.completedTasks / a.totalTasks - b.completedTasks / b.totalTasks;
